...
export class FanPageComponent implements OnInit {
  ...
  changeTeam(index: number, team: string, change: boolean): void {
    if (change === true) {
      if (team === 'madrid') {
        this.barcelonaFans.push(this.madridFans.splice(index, 1)[0]);
      } else {
        this.madridFans.push(this.barcelonaFans.splice(index, 1)[0]);
      }
    }
  }
